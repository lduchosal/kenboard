...
def onboarding_text_full(
    ...
    return (
        ...
        "\n"
        "   ken --help        référence complète\n"
        "   ken help          guide des bonnes pratiques agent\n"
        "\n"
        "## Bonnes pratiques\n"
        "\n"
        "- Workflow : todo → doing → review → done\n"
        "  L'agent gère todo → doing → review. Seul l'utilisateur passe review → done.\n"
        "\n"
        "- Titres de tâches : toujours au format MODULE / Titre\n"
        "  Exemples : AUTH / Login OIDC, BUG / Fix 403, CLEAN / Remove dead code,\n"
        "  SEC / XSS sanitize, UI / Logo login, DOC / INSTALL update\n"
        "\n"
        "- Avant de déclarer le travail terminé :\n"
        "  1. ken move <id> --to review\n"
        "  2. Ajouter un bloc Résolution à la description (ken update --desc)\n"
        "     avec : Modifications, Comportements obtenus, Garde-fous\n"
        "\n"
        "- Toujours utiliser --json quand on parse la sortie\n"
        "- Ne jamais marquer une tâche done soi-même\n"
        "- Le fichier .ken est gitignored (contient un token), ne jamais le committer\n"
        "\n"
        "## Infos\n"
        "\n"
        f"   cat_id     = {cat_id}\n"
